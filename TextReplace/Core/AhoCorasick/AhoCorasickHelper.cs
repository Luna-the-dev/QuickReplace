    ...
    public class AhoCorasickHelper
    {
            ...
            out int numOfMatches)
        {
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                string replacement = (preserveCase) ?
                    SetMatchCase(updatedLine.Substring(matches[i].Position + offset, matches[i].Text.Length), replacePhrases[matches[i].Text]) :
            ...
            out List<int> newRunsToOldRuns)
        {
            ...
            for (int i = 0; i < runs.Count; i++)
            {
                ...
                while (matchIndex < matches.Count && matches[matchIndex].Position < runPtrs[i + 1])
                {
                    ...
                    string replacement = (preserveCase) ?
                        SetMatchCase(paragraphText.Substring(matches[matchIndex].Position, matches[matchIndex].Text.Length), replacePhrases[matches[matchIndex].Text]) :
            ...
            out int numOfMatches)
        {
            ...
            for (int i = 0; i < matches.Count; i++)
            {
                ...
                // preserve the original case of the word that is being replaced if that option is set
                string replacement = (preserveCase) ?
                    SetMatchCase(text.Substring(matches[i].Position, matches[i].Text.Length), replacePhrases[matches[i].Text]) :
                    replacePhrases[matches[i].Text];
                var replaceRunText = new Wordprocessing.Text(replacement)
                {
                    Space = SpaceProcessingModeValues.Preserve
            ...
            out bool wereReplacementsMade)
        {
            ...
            for (int i = 0; i < runs.Count; i++)
            {
                ...
                while (matchIndex < matches.Count && matches[matchIndex].Position < runPtrs[i + 1])
                {
                    ...
                    // preserve the original case of the word that is being replaced if that option is set
                    string replacement = (preserveCase) ?
                        SetMatchCase(cellText.Substring(matches[matchIndex].Position, matches[matchIndex].Text.Length), replacePhrases[matches[matchIndex].Text]) :
                        replacePhrases[matches[matchIndex].Text];
